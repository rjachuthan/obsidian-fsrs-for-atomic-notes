...
## Understanding Window Globals

Most importantly, pop-out windows come with a completely different set of globals. Each pop-out window introduces its own `Window` object, `Document` object, and fresh copies of all global constructors (like `HTMLElement` and `MouseEvent`).

This means that some of the things you previously assumed to be global and use only _a single_ definition, will now only work in the main window. Here are some examples:
...
The Obsidian API includes various helper functions and accessors to better support pop-out windows:
